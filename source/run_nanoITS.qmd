...
You can select what markers you want to analyse in `markers: ["SSU", "ITS1"]`. The workflow was developed for primers targeting both the SSU and ITS1 but the workflow will also run for either option selected and we plan to in the future extend the workflow to also accept the LSU rRNA gene.
...
The most important parameters to check out are the settings for the **filtering** of reads and the minimum length cutoff (**min_its_length**) for the SSU and ITS1 sequences.
