...
rule run_porechop:
    ...
    echo "porechop --input {input} --output {output} --threads {threads} --discard_middle" >> {log}
...
rule run_chopper:
    ...
    echo "gunzip -c {input} | chopper -q {params.quality_score} -l {params.min_length} --maxlength {params.max_length}  --threads {threads}" >> {log}

    gunzip -c {input} |\
            chopper -q {params.quality_score} \
            --headcrop {params.headcrop} --tailcrop {params.tailcrop}   \
            -l {params.min_length} --maxlength {params.max_length}  --threads {threads} |\
            gzip > {output} 
    """
