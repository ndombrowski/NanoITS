...
class ToMat(object):
    ...
    def __init__(self, taxlist, bblastlike, centrifugelike, krakenlike, dbtaxonomy, dbfasta, dbmap):
        ...
        # Extracting information from the pathfile
        parts = self.pathfile.split('/')
        self.dir = '/'.join(parts[:4])
        self.method = parts[3]
        self.sample = parts[4]
        self.marker = parts[5].split('_')[1]
        self.classifier = parts[5].split('_')[2]
        self.outpath = f"{self.dir}/{self.sample}/{self.sample}_{self.marker}_{self.method}"
